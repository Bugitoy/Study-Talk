...
useEffect(() => {
  ...
  const callStateSubscription = useCallback(() => {
    ...
    const sub = call.state.custom$.subscribe((custom: any) => {
      if (custom.quizStarted !== undefined) setQuizStarted(custom.quizStarted);
      ...
      if (custom.quizEnded !== undefined) setQuizEnded(custom.quizEnded);
    });
    ...
    <section className="relative h-screen w-full pt-0 text-white">
      ...
      <div className="relative flex size-full items-center justify-center pb-32 sm:pb-40 md:pb-48 pt-12 sm:pt-24 lg:pt-0">
        <div className="flex flex-col md:flex-row lg:flex-row xl:flex-row items-center gap-0 sm:gap-2 -space-y-2 sm:space-y-0">
          ...
          {/* Show quiz interface when quiz has started */}
          {shouldShowQuiz && (
            <>
              {/* Question sheet */}
              ...
              <div className="flex flex-col items-center justify-center w-full max-w-[25rem] sm:max-w-[30rem] md:max-w-[20rem] lg:max-w-[35rem] xl:max-w-[25rem] 2xl:max-w-[50rem] 3xl:max-w-[60rem] h-[15rem] sm:h-[20rem] md:h-[15rem] lg:h-[24rem] xl:h-[20rem] 2xl:h-[25rem] 3xl:h-[30rem] mx-auto gap-2 sm:gap-5 md:gap-2 lg:gap-2 xl:gap-5">
                ...
                <button
                  ...
                  disabled={timeLeft === Infinity && selectedAnswer !== null}
                  className={cn(
                    "bg-thanodi-peach border border-gray-300 rounded-[30px] shadow-md flex items-center justify-center text-xs sm:text-xl md:text-sm lg:text-xl xl:text-lg 2xl:text-2xl 3xl:text-3xl font-bold text-gray-600 p-2 sm:p-5 md:p-3 lg:p-5 xl:p-4 2xl:p-6 3xl:p-8 w-full",
      ...
      <div className="fixed bottom-0 left-0 right-0 rounded-t-xl flex w-full items-center justify-center gap-1 sm:gap-2 md:gap-3 lg:gap-4 flex-nowrap p-1 sm:p-2 md:p-4 bg-black/20 backdrop-blur-sm overflow-x-auto">
        <div className="flex items-center gap-1 sm:gap-2 md:gap-3">
          ...
          {roomSettings?.mic === 'flexible' && call && (
            <button
              ...
              className="cursor-pointer rounded-2xl bg-[#19232d] px-1 sm:px-2 md:px-4 py-1 sm:py-2 hover:bg-[#4c535b] flex items-center justify-center"
            >
              {call.microphone.state.status === 'enabled' ? (
                <Mic size={14} className="sm:w-4 md:w-5 text-white" />
              ) : (
                <MicOff size={14} className="sm:w-4 md:w-5 text-white" />
              )}
            </button>
          )}
          {roomSettings?.camera === 'flexible' && call && (
            ...
            >
              {call.camera.state.status === 'enabled' ? (
                <Video size={14} className="sm:w-4 md:w-5 text-white" />
              ) : (
                <VideoOff size={14} className="sm:w-4 md:w-5 text-white" />
              )}
            </button>
          )}
          <button
            onClick={handleLeaveCall}
            className="cursor-pointer rounded-2xl bg-red-500 px-1 sm:px-2 md:px-4 py-1 sm:py-2 hover:bg-red-600 flex items-center justify-center"
          >
            <PhoneOff size={14} className="sm:w-4 md:w-5 text-white" />
          </button>
        </div>
        ...
        {shouldShowStartButton && (
          <button
            ...
            className="cursor-pointer rounded-2xl bg-[#19232d] px-1 sm:px-2 md:px-4 py-1 sm:py-2 hover:bg-[#4c535b] rounded-2xl shadow-md flex items-center justify-center text-[10px] sm:text-xs md:text-sm text-white"
          >
        ...
        <button onClick={() => dispatch({ type: 'SET_PARTICIPANTS', payload: true })}>
          <div className="cursor-pointer rounded-2xl bg-[#19232d] px-1 sm:px-2 md:px-4 py-1 sm:py-2 hover:bg-[#4c535b] flex items-center justify-center">
            <Users size={14} className="sm:w-4 md:w-5 text-white" />
          </div>
        </button>
        ...
        <button onClick={handleOpenReportDialog}>
          <div className="cursor-pointer rounded-2xl bg-[#19232d] px-1 sm:px-2 md:px-4 py-1 sm:py-2 hover:bg-red-600 flex items-center justify-center">
            <Flag size={14} className="sm:w-4 md:w-5 text-white" />
          </div>
        </button>
        ...
        {isHost && (
          <button onClick={handleOpenBanDialog}>
            <div className="cursor-pointer rounded-2xl bg-[#19232d] px-1 sm:px-2 md:px-4 py-1 sm:py-2 hover:bg-red-600 flex items-center justify-center">
              <Shield size={14} className="sm:w-4 md:w-5 text-white" />
            </div>
          </button>
        )}
